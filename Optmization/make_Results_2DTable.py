...
HiggsMass = "VisibleMass_"
#OriginRootDir = '../NewResults/'
OriginRootDir = '../ROOTFiles/Results/'
SubRootDir = 'outRoot/'
luminosity = 19242
def XSection(mX):
    ...
    for chl in range(len(channel)):
        ...
        for sig in range(len(signal)):
            for m in range(len(mass)):#    for m in range(110, 145, 5):
                ...
                HistoSub = myfileSub.Get(HiggsMass+str(channel[chl])+"_pp" )
                value = 0
        ...
        for bg in range (len(BackGround)):
            ...
            HistoSubB = myfileSub.Get(HiggsMass+str(channel[chl])+"_pp")
