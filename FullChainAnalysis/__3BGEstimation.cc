...
TFile * PrintResults(TH2F* Chnl_estimate, TH2F* Chnl_event) {
    ...
    //    cout << "MMTT Observed = " << Hist2_PP_MMTT->Integral() << "\t EETT Observed = " << Hist2_PP_EETT->Integral() << "\n";
...
void Measure(int num, std::vector<float> p, bool p_ptDep, std::vector<float> q, bool q_ptDep, TH2F * Chnl_estimate, TH2F* Chnl_event, std::string ff, std::string fp, std::string pf) {
        TFile * MegaFile = new TFile("outRoot/Data.root");
//    TFile * MegaFile = new TFile("mega.root");

    cout << MegaFile->GetName() << endl;
    cout << "ff=  " << ff << "   fp=  " << fp << "   pf=  " << pf << "\n";
    TH2D *Hist2_FF = (TH2D*) MegaFile->Get(ff.c_str());
    cout << Hist2_FF->GetName() << endl;
    ...
    for (int i = 10; i < Hist2_PF->GetNbinsX(); i++) {
        ...
        //        cout << Value << " ########### " << FrateX << " ########### " << Frate << " ########### " << q[0] << " " << q[1] << " " << q[2] << "\n";
    }
}
...
void __3BGEstimation() {
    ...
    Get_FitParameter(2, "tauMedium", 1, "eleLoose", 0, Chnl_estimate, Chnl_event, "FakeRate_MMET_apply_ff", "FakeRate_MMET_apply_fp", "FakeRate_MMET_apply_pf");
    Get_FitParameter(3, "tauMedium", 1, "muLoose", 0, Chnl_estimate, Chnl_event, "FakeRate_MMMT_apply_ff", "FakeRate_MMMT_apply_fp", "FakeRate_MMMT_apply_pf");
    ...
    Get_FitParameter(6, "tauMedium", 1, "muLoose", 0, Chnl_estimate, Chnl_event, "FakeRate_EEMT_apply_ff", "FakeRate_EEMT_apply_fp", "FakeRate_EEMT_apply_pf");
    Get_FitParameter(7, "tauMedium", 1, "eleLoose", 0, Chnl_estimate, Chnl_event, "FakeRate_EEET_apply_ff", "FakeRate_EEET_apply_fp", "FakeRate_EEET_apply_pf");
