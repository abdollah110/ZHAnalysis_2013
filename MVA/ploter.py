...
if __name__ == "__main__":
    ...
    file_ZH = TFile("preMVAZH.root", "OPEN")
    file_ZZ = TFile("preMVAZZ.root", "OPEN")
    file_Red = TFile("preMVARed.root", "OPEN")
    file_Data = TFile("preMVAData.root", "OPEN")


    Hist_Name = "mva_N_LT"
    Hist_ZH = file_ZH.Get(Hist_Name)
    Hist_ZZ = file_ZZ.Get(Hist_Name)
    Hist_Red = file_Red.Get(Hist_Name)
    Hist_data = file_Data.Get(Hist_Name)


    Hist_ZH.SetMarkerColor(2)
    Hist_ZH.DrawNormalized()
    Hist_ZZ.DrawNormalized("same")
    Hist_Red.DrawNormalized("same")
...
#    hist_Estimation3.Draw("same")
    ...
    l.SetFillColor(0)
    l.SetTextSize(.03)
    l.AddEntry(Hist_ZH, "ZHtautau125", "p")
    l.AddEntry(Hist_ZZ, "ZZ", "lpf")
    l.AddEntry(Hist_Red, "Reducible", "lpf")
    l.AddEntry(Hist_Red, "Data", "lpf")
    l.Draw()
    ...
    t.DrawLatex(0.5, .52, "Data =    Estimation = "  )
