...
int main(int argc, char** argv) {
    ...
    //###############################################################################################
    //REading from Pavel Tree
    UInt_t Run_p = 0;
    UInt_t Lumi_p = 0;
    UInt_t Event_p = 0;
    Short_t Channel_p;
    Short_t subChannel_p;
    TFile * PavelFile = new TFile("DoubleEle_syncTree.root");
    TFile * PavelFileMu = new TFile("DoubleMu_syncTree.root");
    ...
    for (Int_t i = 0; i < nentries_wtn; i++) {
        ...
        //###########################        Optimum    #################################################
        //###############################################################################################
        bool tau3_IsoB = l3_tauIso3HitM;
        bool tau4_IsoB = l4_tauIso3HitM;
        bool tau4_Iso_ltau = l4_tauIso3HitL;
        float optimum_Lt_TT = 70;
        float optimum_Lt_MT = 45;
        float optimum_Lt_ET = 30;
        float optimum_Lt_EM = 25;
        ...
        if ((Channel == 91) && l3Pt > CutOnTauPt && l4Pt > CutOnTauPt && l3_tauRejMu2L && l3_tauRejEleL && l4_tauRejMu2L && l4_tauRejEleL) {
            ...
            if (LTcut_tt && charge_Type && !tau3_IsoB && !tau4_IsoB && (Event != Event_Double[1][1])) {
                fillTreeN(BG_Tree, Channel, 0, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
                Event_Double[1][1] = Event;
            }
            if (LTcut_tt && charge_Type && tau3_IsoB && !tau4_IsoB && (Event != Event_Double[2][1])) {
                fillTreeN(BG_Tree, Channel, 1, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
                Event_Double[2][1] = Event;
            }
            if (LTcut_tt && charge_Type && !tau3_IsoB && tau4_IsoB && (Event != Event_Double[3][1])) {
                fillTreeN(BG_Tree, Channel, 2, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
            ...
            if (LTcut_tt && charge_Type && tau3_IsoB && tau4_IsoB && (Event != Event_Double[0][1])) {
                ...
                fillTreeN(BG_Tree, Channel, 3, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
                Event_Double[0][1] = Event;

                plotFill("__VisibleMass_ZH", VisibleMass, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
                plotFill("__SVMass_ZH", SVMass, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
                plotFill("__pu_Weight_ZH", pu_Weight, 100, 0, 5);
                plotFill("__eff_Correction_ZH", eff_Correction, 100, 0, 5);
                plotFill("__pu_Weight_TIMES_eff_Correction_ZH", pu_Weight * eff_Correction, 100, 0, 5);
            ...
            if (LTcut_tt && SS && l3_tauIsoMVA2raw > value_looserTauIso && l4_tauIsoMVA2raw > value_looserTauIso) {
                ...
                fillTreeN(BG_Tree, Channel, 4, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
            }
        }
        // ****************************************************************************
        // ScaleUp TT
        ...
        //####################################################
//        if ( Event == 30641) {
        if (Run == 1 && Event == 30641 && Lumi ==82 ) {
            cout << "\nChannel = " << Channel << "  Run =" << Run << "  Event =" << Event << "  Lumi =" << Lumi << " pu_Weight=" << pu_Weight << "  eff_Correction=" << eff_Correction << "  Tot=" << pu_Weight * eff_Correction << endl;
            cout << "L1=" << l1Pt << " L2="<<l2Pt<<" L3="<<l3Pt<<" L4="<<l4Pt<<endl;
            cout << "L1=" << l1Eta << " L2="<<l2Eta<<" L3="<<l3Eta<<" L4="<<l4Eta<<endl;
        }
        //
        //        if (Channel == 92) {
        //            TTree * PavelTreeMu = (TTree*) PavelFileMu->Get("UCL_tree");
        //            PavelTreeMu->SetBranchAddress("Run_", &Run_p);
        //            PavelTreeMu->SetBranchAddress("Event_", &Event_p);
        //            PavelTreeMu->SetBranchAddress("Lumi_", &Lumi_p);
        //            PavelTreeMu->SetBranchAddress("Channel_", &Channel_p);
        //            PavelTreeMu->SetBranchAddress("subChannel_", &subChannel_p);
        //            Int_t nentries_wtn = (Int_t) PavelTreeMu->GetEntries();
        //            for (Int_t i = 0; i < nentries_wtn; i++) {
        //                PavelTreeMu->GetEntry(i);
        //                //                cout << Channel << " *******= " << Channel_p << "\n";
        //                if (Channel - 90 == Channel_p && Run == Run_p && Event == Event_p && Lumi == Lumi_p) {
        //
        //                    if (subChannel_p == 0) cout << "Channel = " << Channel_p << "  Run =" << Run_p << "  Event =" << Event_p << "  Lumi =" << Lumi_p << "  subChannel =" << subChannel_p << "\t" << LTcut_et << charge_Type << !tau4_Iso_ltau << (!l3_eleId_Tight || !(l3_eleIso < TightEle)) << " _ " << (mu_Size == 2) << l4_tauRejEleMVA3T << l4_tauRejMu2L << "\n";
        //                    if (subChannel_p == 1) cout << "Channel = " << Channel_p << "  Run =" << Run_p << "  Event =" << Event_p << "  Lumi =" << Lumi_p << "  subChannel =" << subChannel_p << "\t" << LTcut_et << charge_Type << !tau4_Iso_ltau << (l3_eleId_Tight && l3_eleIso < TightEle) << " _ " << (mu_Size == 2) << l4_tauRejEleMVA3T << l4_tauRejMu2L << "\n";
        //                    if (subChannel_p == 2) cout << "Channel = " << Channel_p << "  Run =" << Run_p << "  Event =" << Event_p << "  Lumi =" << Lumi_p << "  subChannel =" << subChannel_p << "\t" << LTcut_et << charge_Type << tau4_Iso_ltau << (!l3_eleId_Tight || !(l3_eleIso < TightEle)) << " _ " << (mu_Size == 2) << l4_tauRejEleMVA3T << l4_tauRejMu2L << "\n";
        //                    cout << "\n";
        //                }
        //            }
        //        }
        if (Channel == 92 && l4Pt > CutOnTauPt && mu_Size == 2 && l4_tauRejEleMVA3T && l4_tauRejMu2L) {
            ...
            if (LTcut_et && charge_Type && !tau4_Iso_ltau && (!l3_eleId_Tight || !(l3_eleIso < TightEle)) && (Event != Event_Double[1][2])) {
                fillTreeN(BG_Tree, Channel, 0, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
                Event_Double[1][2] = Event;
            }
            if (LTcut_et && charge_Type && !tau4_Iso_ltau && (l3_eleId_Tight && l3_eleIso < TightEle) && (Event != Event_Double[2][2])) {
                fillTreeN(BG_Tree, Channel, 1, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
                Event_Double[2][2] = Event;
            }
            if (LTcut_et && charge_Type && tau4_Iso_ltau && (!l3_eleId_Tight || !(l3_eleIso < TightEle)) && (Event != Event_Double[3][2])) {
                fillTreeN(BG_Tree, Channel, 2, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
                Event_Double[3][2] = Event;
            }
            if (LTcut_et && charge_Type && tau4_Iso_ltau && (l3_eleId_Tight && (l3_eleIso < TightEle)) && (Event != Event_Double[0][2]) && electron_Size == 1) {
        ...
        if (Channel == 92 && l4Pt * scaleUp > CutOnTauPt && mu_Size == 2 && l4_tauRejEleMVA3T && l4_tauRejMu2L) {
            ...
            if (LTcut_et && charge_Type && tau4_Iso_ltau && (l3_eleId_Tight && (l3_eleIso < TightEle)) && (Event != Event_Double[5][2]) && electron_Size == 1) {
        ...
        if (Channel == 92 && l4Pt * scaleDown > CutOnTauPt && mu_Size == 2 && l4_tauRejEleMVA3T && l4_tauRejMu2L) {
            ...
            if (LTcut_et && charge_Type && tau4_Iso_ltau && (l3_eleId_Tight && (l3_eleIso < TightEle)) && (Event != Event_Double[6][2]) && electron_Size == 1) {
        ...
        if (Channel == 93 && l4Pt > CutOnTauPt && electron_Size == 0 && l4_tauRejEleL && l4_tauRejMu2T) {
            ...
            if (LTcut_mt && charge_Type && !tau4_Iso_ltau && (!l3_muId_Tight || !(l3_muIso < TightMu)) && (Event != Event_Double[1][3])) {
                fillTreeN(BG_Tree, Channel, 0, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
                Event_Double[1][3] = Event;
            }
            if (LTcut_mt && charge_Type && !tau4_Iso_ltau && (l3_muId_Tight && l3_muIso < TightMu) && (Event != Event_Double[2][3])) {
                fillTreeN(BG_Tree, Channel, 1, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
                Event_Double[2][3] = Event;
            }
            if (LTcut_mt && charge_Type && tau4_Iso_ltau && (!l3_muId_Tight || !(l3_muIso < TightMu)) && (Event != Event_Double[3][3])) {
                fillTreeN(BG_Tree, Channel, 2, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
                Event_Double[3][3] = Event;
            }
            if (LTcut_mt && charge_Type && tau4_Iso_ltau && (l3_muId_Tight && (l3_muIso < TightMu)) && (Event != Event_Double[0][3]) && mu_Size == 3) {
                plotFill("VisibleMass_lt_pp", VisibleMass, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
        ...
        if (Channel == 93 && l4Pt * scaleUp > CutOnTauPt && electron_Size == 0 && l4_tauRejEleL && l4_tauRejMu2T) {
            ...
            if (LTcut_mt && charge_Type && tau4_Iso_ltau && (l3_muId_Tight && (l3_muIso < TightMu)) && (Event != Event_Double[5][3]) && mu_Size == 3) {
        ...
        if (Channel == 93 && l4Pt * scaleDown > CutOnTauPt && electron_Size == 0 && l4_tauRejEleL && l4_tauRejMu2T) {
            ...
            if (LTcut_mt && charge_Type && tau4_Iso_ltau && (l3_muId_Tight && (l3_muIso < TightMu)) && (Event != Event_Double[6][3]) && mu_Size == 3) {
        ...
        if (Channel == 96 && l4Pt > CutOnTauPt && electron_Size == 2 && l4_tauRejEleL && l4_tauRejMu2T) {
            ...
            if (LTcut_mt && charge_Type && !tau4_Iso_ltau && (!l3_muId_Tight || !(l3_muIso < TightMu)) && (Event != Event_Double[1][6])) {
                fillTreeN(BG_Tree, Channel, 0, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
                Event_Double[1][6] = Event;
            }
            if (LTcut_mt && charge_Type && !tau4_Iso_ltau && (l3_muId_Tight && l3_muIso < TightMu) && (Event != Event_Double[2][6])) {
                fillTreeN(BG_Tree, Channel, 1, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
                Event_Double[2][6] = Event;
            }
            if (LTcut_mt && charge_Type && tau4_Iso_ltau && (!l3_muId_Tight || !(l3_muIso < TightMu)) && (Event != Event_Double[3][6])) {
                fillTreeN(BG_Tree, Channel, 2, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
                Event_Double[3][6] = Event;
            }
            if (LTcut_mt && charge_Type && tau4_Iso_ltau && (l3_muId_Tight && (l3_muIso < TightMu)) && (Event != Event_Double[0][6]) && mu_Size == 1) {
                plotFill("VisibleMass_lt_pp", VisibleMass, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
        ...
        if (Channel == 96 && l4Pt * scaleUp > CutOnTauPt && electron_Size == 2 && l4_tauRejEleL && l4_tauRejMu2T) {
            ...
            if (LTcut_mt && charge_Type && tau4_Iso_ltau && (l3_muId_Tight && (l3_muIso < TightMu)) && (Event != Event_Double[5][6]) && mu_Size == 1) {
        ...
        if (Channel == 96 && l4Pt * scaleDown > CutOnTauPt && electron_Size == 2 && l4_tauRejEleL && l4_tauRejMu2T) {
            ...
            if (LTcut_mt && charge_Type && tau4_Iso_ltau && (l3_muId_Tight && (l3_muIso < TightMu)) && (Event != Event_Double[6][6]) && mu_Size == 1) {
        ...
        //        if (Channel == 97) {
        //            TTree * PavelTree = (TTree*) PavelFile->Get("UCL_tree");
        //            PavelTree->SetBranchAddress("Run_", &Run_p);
        //            PavelTree->SetBranchAddress("Event_", &Event_p);
        //            PavelTree->SetBranchAddress("Lumi_", &Lumi_p);
        //            PavelTree->SetBranchAddress("Channel_", &Channel_p);
        //            PavelTree->SetBranchAddress("subChannel_", &subChannel_p);
        //            Int_t nentries_wtn = (Int_t) PavelTree->GetEntries();
        //            for (Int_t i = 0; i < nentries_wtn; i++) {
        //                PavelTree->GetEntry(i);
        //                //                cout << Channel << " *******= " << Channel_p << "\n";
        //                if (Channel - 90 == Channel_p && Run == Run_p && Event == Event_p && Lumi == Lumi_p) {
        //
        //                    if (subChannel_p == 0) cout << "Channel = " << Channel_p << "  Run =" << Run_p << "  Event =" << Event_p << "  Lumi =" << Lumi_p << "  subChannel =" << subChannel_p << "\t" << LTcut_et << charge_Type << !tau4_Iso_ltau << (!l3_eleId_Tight || !(l3_eleIso < TightEle)) << " _ " << (mu_Size == 0) << l4_tauRejEleMVA3T << l4_tauRejMu2L << "\n";
        //                    if (subChannel_p == 1) cout << "Channel = " << Channel_p << "  Run =" << Run_p << "  Event =" << Event_p << "  Lumi =" << Lumi_p << "  subChannel =" << subChannel_p << "\t" << LTcut_et << charge_Type << !tau4_Iso_ltau << (l3_eleId_Tight && l3_eleIso < TightEle) << " _ " << (mu_Size == 0) << l4_tauRejEleMVA3T << l4_tauRejMu2L << "\n";
        //                    if (subChannel_p == 2) cout << "Channel = " << Channel_p << "  Run =" << Run_p << "  Event =" << Event_p << "  Lumi =" << Lumi_p << "  subChannel =" << subChannel_p << "\t" << LTcut_et << charge_Type << tau4_Iso_ltau << (!l3_eleId_Tight || !(l3_eleIso < TightEle)) << " _ " << (mu_Size == 0) << l4_tauRejEleMVA3T << l4_tauRejMu2L << "\n";
        //                    cout << "\n";
        //                }
        //            }
        //        }
        ...
        if (Channel == 97 && l4Pt > CutOnTauPt && mu_Size == 0 && l4_tauRejEleMVA3T && l4_tauRejMu2L) {
            ...
            if (LTcut_et && charge_Type && !tau4_Iso_ltau && (!l3_eleId_Tight || !(l3_eleIso < TightEle)) && (Event != Event_Double[1][7])) {
                fillTreeN(BG_Tree, Channel, 0, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
                Event_Double[1][7] = Event;
            }
            if (LTcut_et && charge_Type && !tau4_Iso_ltau && (l3_eleId_Tight && l3_eleIso < TightEle) && (Event != Event_Double[2][7])) {
                fillTreeN(BG_Tree, Channel, 1, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
                Event_Double[2][7] = Event;
            }
            if (LTcut_et && charge_Type && tau4_Iso_ltau && (!l3_eleId_Tight || !(l3_eleIso < TightEle)) && (Event != Event_Double[3][7])) {
                fillTreeN(BG_Tree, Channel, 2, VisibleMass, SVMass, Run, Lumi, Event, l3Pt, l3Eta, l3_CloseJetPt, l3_CloseJetEta, l4Pt, l4Eta, l4_CloseJetPt, l4_CloseJetEta);
                Event_Double[3][7] = Event;
            }
            if (LTcut_et && charge_Type && tau4_Iso_ltau && (l3_eleId_Tight && (l3_eleIso < TightEle)) && (Event != Event_Double[0][7]) && electron_Size == 3) {
                plotFill("VisibleMass_lt_pp", VisibleMass, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
        ...
        if (Channel == 97 && l4Pt * scaleUp > CutOnTauPt && mu_Size == 0 && l4_tauRejEleMVA3T && l4_tauRejMu2L) {
            ...
            if (LTcut_et && charge_Type && tau4_Iso_ltau && (l3_eleId_Tight && (l3_eleIso < TightEle)) && (Event != Event_Double[5][7]) && electron_Size == 3) {
        ...
        if (Channel == 97 && l4Pt * scaleDown > CutOnTauPt && mu_Size == 0 && l4_tauRejEleMVA3T && l4_tauRejMu2L) {
            ...
            if (LTcut_et && charge_Type && tau4_Iso_ltau && (l3_eleId_Tight && (l3_eleIso < TightEle)) && (Event != Event_Double[6][7]) && electron_Size == 3) {
        ...
        if ((Channel == 91 || Channel == 95) && SS && l3Pt + l4Pt > 50 && l3Pt > CutOnTauPt && l4Pt > CutOnTauPt && l3_tauRejMu2L && l3_tauRejEleL && l4_tauRejMu2L && l4_tauRejEleL && Event != Ev_double_tau) {
            ...
            //#########################
            //  XXXXXXXXXXXX BUG l3Eta should be  ----------> fabs(l3Eta)
            //  XXXXXXXXXXXX BUG l4Eta should be  ----------> fabs(l4Eta)
            //Barrel Fake rate
            ...
            //Barrel Fake rate
            if (FR_Estimation && fabs(l3_CloseJetEta) < 1.4) plotFill("FakeRate_TT_Tau_Pt_Before_CloseJet_B", l3_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
            if (FR_Estimation && fabs(l4_CloseJetEta) < 1.4) plotFill("FakeRate_TT_Tau_Pt_Before_CloseJet_B", l4_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
            if (FR_Estimation && tau3_IsoB && fabs(l3_CloseJetEta) < 1.4) plotFill("FakeRate_TT_Tau_Pt_After_Loose_CloseJet_B", l3_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
            if (FR_Estimation && tau4_IsoB && fabs(l4_CloseJetEta) < 1.4) plotFill("FakeRate_TT_Tau_Pt_After_Loose_CloseJet_B", l4_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
            //EndCap Fake rate
            if (FR_Estimation && fabs(l3_CloseJetEta) > 1.4) plotFill("FakeRate_TT_Tau_Pt_Before_CloseJet_E", l3_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
            if (FR_Estimation && fabs(l4_CloseJetEta) > 1.4) plotFill("FakeRate_TT_Tau_Pt_Before_CloseJet_E", l4_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
            if (FR_Estimation && tau3_IsoB && fabs(l3_CloseJetEta) > 1.4) plotFill("FakeRate_TT_Tau_Pt_After_Loose_CloseJet_E", l3_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
            if (FR_Estimation && tau4_IsoB && fabs(l4_CloseJetEta) > 1.4) plotFill("FakeRate_TT_Tau_Pt_After_Loose_CloseJet_E", l4_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
        ...
        if (lepTauFR_denum && Event != Ev_double_Ltau) {
            ...
            if (lepTauFR_denum && fabs(l4_CloseJetEta) < 1.4) plotFill("FakeRate_LT_Tau_Pt_Before_CloseJet_B", l4_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
            if (lepTauFR_denum && tau4_Iso_ltau && fabs(l4_CloseJetEta) < 1.4) plotFill("FakeRate_LT_Tau_Pt_After_Loose_CloseJet_B", l4_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);

            if (lepTauFR_denum && fabs(l4_CloseJetEta) > 1.4) plotFill("FakeRate_LT_Tau_Pt_Before_CloseJet_E", l4_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
            if (lepTauFR_denum && tau4_Iso_ltau && fabs(l4_CloseJetEta) > 1.4) plotFill("FakeRate_LT_Tau_Pt_After_Loose_CloseJet_E", l4_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
        ...
        if (Mu_denum) {
            plotFill("3objFR_Mu_Denum", l3_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
        }
        if (Mu_denum && l3_muId_Tight && l3_muIso < TightMu) {
            plotFill("3objFR_Mu_NumTight", l3_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
        }
        if (Mu_denum && l3_muId_Loose && l3_muIso < looseLep) {
            plotFill("3objFR_Mu_NumLoose", l3_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
        }
        if (Ele_denum) {
            plotFill("3objFR_Ele_Denum", l3_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
        }
        if (Ele_denum && l3_eleId_Tight && l3_eleIso < TightEle) {
            plotFill("3objFR_Ele_NumTight", l3_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
        }
        if (Ele_denum && l3_eleId_Loose && l3_eleIso < looseLep) {
            plotFill("3objFR_Ele_NumLoose", l3_CloseJetPt, pt_Bin, 0, pt_Bin, pu_Weight * eff_Correction);
        }
