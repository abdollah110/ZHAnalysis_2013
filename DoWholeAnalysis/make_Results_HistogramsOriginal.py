...
def MakeTheHistogram(histoName,CoMEnergy):
    ...
    for chl in range(len(channel)):
        ...
        for bg in range (len(BackGround)):
            ...
            myfile = TFile(DIR_ROOT + str(BackGround[bg]) + CoMEnergy+'.root')
            Histo = myfile.Get(histoName + str(channel[chl])+"_pp")
            Histo.Rebin(reb_)
            ...
            myfile = TFile(DIR_ROOT + str(BackGround[bg]) +CoMEnergy+ '.root')
            HistoUp = myfile.Get(histoName + str(channel[chl])+"_pp_Up")
            HistoUp.Rebin(reb_)
            for bb in range(0,n_bin):
                NewHistUp.SetBinContent(bb,HistoUp.GetBinContent(bb))
                NewHistUp.SetBinError(bb,HistoUp.GetBinError(bb))
            myfile.Close()
            ...
            myfile = TFile(DIR_ROOT + str(BackGround[bg]) + CoMEnergy+'.root')
            HistoDown = myfile.Get(histoName + str(channel[chl])+"_pp_Down")
            HistoDown.Rebin(reb_)
            for bb in range(0,n_bin):
                NewHistDown.SetBinContent(bb,HistoDown.GetBinContent(bb))
                NewHistDown.SetBinError(bb,HistoDown.GetBinError(bb))
            myfile.Close()
