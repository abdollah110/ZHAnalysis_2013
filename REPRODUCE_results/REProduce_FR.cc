...
//    Run_Tree->SetBranchAddress("ele_partLoose_Size", &ele_partLoose_Size);
    ...
    for (Int_t i = 0; i < nentries_wtn; i++) {
        ...
        if (Channel == 52 || Channel == 57) {
            plotFill("ET_Electron_Pt_Before_0", l3Pt, pt_Bin, 0, pt_Bin);
            if (l3_eleId && l3_eleIso < 0.30) plotFill("ET_Electron_Loose_After_0", l3Pt, pt_Bin, 0, pt_Bin);
            if (l3_eleId && l3_eleIso < 0.10) plotFill("ET_Electron_Tight_After_0", l3Pt, pt_Bin, 0, pt_Bin);
        }

        if (Channel == 53 || Channel == 56) {
            plotFill("MT_Muon_Pt_Before_0", l3Pt, pt_Bin, 0, pt_Bin);
            if (l3_muId && l3_muIso < 0.30) plotFill("MT_Muon_Loose_After_0", l3Pt, pt_Bin, 0, pt_Bin);
            if (l3_muId && l3_muIso < 0.15) plotFill("MT_Muon_Tight_After_0", l3Pt, pt_Bin, 0, pt_Bin);
        }
