...
void 
HTT_ET_X(bool scaled=true, bool log=false, float min=0., float max=-1., const char* inputfile="root/$HISTFILE", const char* directory="$CATEGORY")
{
...
//  }
  ...
  chan->SetTextColor(    1 );
  chan->SetTextFont (   62 );
  if (directory=="eeet_zh")
  chan->AddText("eee#tau");
  else
  chan->AddText("#mu#mue#tau");
  chan->Draw();
